# MORPHEUS-X Person 2: Intelligence Engine Documentation

## 📋 Overview

Person 2 is responsible for building the **Intelligence Engine** - the brain that analyzes malware DNA and produces professional threat intelligence.

Your modules receive raw analysis data from **Person 1** (Static Analyzer) and produce:
...
```json
{
  ...
  "tactics_involved": ["Execution", "Persistence", "Defense Evasion", "Command and Control"]
}
```
...
1. Risk scoring (100/100 = CRITICAL)
2. Behavior prediction (5 behaviors detected)
3. MITRE mapping (7 techniques)
4. Recommendations (7 actions)
5. JSON output
...
- **Input**: Person 1's `analyzer.py` output (JSON)
- **Output**: Goes to Person 3 (YARA generator) and Person 4 (web dashboard)
...
- Person 3 uses your intelligence output for YARA generation
- Person 4 displays your findings in the web dashboard
- Person 4 includes your recommendations in reports
